...
class AgreementDAO:
    ...
    def create(self, db: Session, agreement_entity: AgreementEntity) -> AgreementModel:
        ...
        try:
            agreement = AgreementModel(
                ...
                user_id=agreement_entity.get_idExternalUser(),
                project_id=agreement_entity.get_idProject()
            )
            db.add(agreement)
            db.commit()
    ...
    def update(self, db: Session, agreement_entity: AgreementEntity) -> AgreementModel:
        ...
        try:
            ...
            agreement.start_date = agreement_entity.get_startDate()
            agreement.end_date = agreement_entity.get_endDate()
            agreement.status = agreement_entity.get_status.value
            agreement.user_id = agreement_entity.get_idExternalUser()
            agreement.project_id = agreement_entity.get_idProject()
    ...
    def list(self, db: Session) -> List[AgreementModel]:
        ...
        try:
            ...
            raise ValueError(f"No se pudieron listar los convenios o no hay convenios que listar: {e}")
